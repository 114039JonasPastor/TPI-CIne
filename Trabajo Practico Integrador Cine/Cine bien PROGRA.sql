...
ALTER procedure [dbo].[SP_CONSULTAR_CLASIFICACIONES]
AS
BEGIN
	SELECT * FROM CLASIFICACIONES
END;
...
@id_idioma int
as
select titulo Titulo, sinopsis Sinopsis,clasificacion Clasificacion, genero Genero, idioma Idioma 
from PELICULAS p
join GENEROS g on g.id_genero = p.id_genero
join IDIOMAS i on i.id_idioma = p.id_idioma
join CLASIFICACIONES c on c.id_clasificacion = p.id_clasificacion
...
CREATE PROC SP_COMBO_PELICULAS
AS
SELECT * FROM PELICULAS

ALTER proc [dbo].[SP_CONSULTAR_PELICULAS_SIN_FILTRO]
as
select id_pelicula ID,titulo Titulo, duracion Duracion,clasificacion Clasificacion, genero Genero, idioma Idioma, estado Estado 
from PELICULAS p
join GENEROS g on g.id_genero = p.id_genero
join IDIOMAS i on i.id_idioma = p.id_idioma
join CLASIFICACIONES c on c.id_clasificacion = p.id_clasificacion

alter table horarios drop column horario
alter table horarios add horario varchar(200)
...
join SALAS s on s.id_sala = f.id_sala
join Horarios h on h.id_horario = f.id_horario
where f.id_funcion = @id_funcion or (fecha_desde >= @desde and fecha_hasta <= @hasta)

exec SP_CONSULTAR_FUNCIONES_FILTROS 1,'2020-10-11','2023-02-01'
